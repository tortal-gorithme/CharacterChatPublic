...
import keyboard
import os
import screenshot_thingy as sct
...
class SpeechToTextManager:
    ...
    def __init__(self):
        ...
        try:
            self.azure_speechconfig = speechsdk.SpeechConfig(subscription=azureKey, region=azureRegion)
        except RuntimeError:
            exit("Ooops! You forgot to set AzureKey and AzureRegion ! Go change that in super_duper_private")
        
        self.azure_speechconfig.speech_recognition_language="fr-FR"
    ...
    def speechtotext_from_mic_continuous_with_options(self, stop_key='p', thinking_key='t', image_key='i'):
        self.azure_speechrecognizer = speechsdk.SpeechRecognizer(speech_config=self.azure_speechconfig)

        pass_image = False
        activate_thinking = False
        done = False
        ...
        while not done:
            if keyboard.read_key() == thinking_key:
                if activate_thinking == False :
                    print("\nThinking mode activated ! The AI will now think before speaking. (Press again to toggle)\n")
                    activate_thinking = True
                else :
                    print("\nThinking mode deactivated ! (Press again to toggle)\n")
                    activate_thinking = False
            if keyboard.read_key() == image_key:
                if pass_image == False :
                    print("\n Image mode activated ! The AI will now see the latest screenshot. (Press again to toggle)\n")
                    pass_image = True
                    image_to_send = sct.TakeTheShot()
                    print("\n Screenshot has been taken !\n")
                else :
                    print("\n Image mode deactivated !\n")
                    pass_image = False
            # METHOD 1 - Press the stop key. This is 'p' by default but user can provide different key
            if keyboard.read_key() == stop_key:
                print("\nEnding azure speech recognition\n")
        ...
        if pass_image == False :
            image_to_send = None
        return {'result' : final_result,'thinking' : activate_thinking,'image' : pass_image, 'image_ref' : image_to_send}
